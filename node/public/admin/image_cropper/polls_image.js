jQuery(document).ready(function ($) {
	jQuery("#poll_banner_crop").cropzeePollBanner();
});
...
jQuery.fn.extend({
	cropzeePollBanner: function (options = {
		// croppr.js options
		// see https://jamesooi.design/Croppr.js/
		aspectRatio	: 0.5,
		maxSize		: null,
		minSize		: [100, 100, 'px'],
		startSize	: [600, 270, 'px'],
		onCropStart	: null,
		onCropMove	: null,
		onCropEnd	: null,
		onInitialize: null,
		...
		window.cropzeePollBannerCreateImage = function (id) {
			 
			$("#pollbanner-hidden-canvas").show();
			$(".old_image_hide").hide();
			...
			var canvas = document.getElementById('pollbanner-hidden-canvas');
			...
			if (options.returnImageMode == 'blob') {
				...
				$("#poll_image_binary").val(dataURL);
				$("#thumbnail_banner_video_image").val('');
				$("#profile_vedio_css").hide();
			}
		...
		function cropzeeTriggerModal(id, src) {
			...
			var lightmodalHTML =
					...
					+ '<div onclick="cropzeePollBannerCreateImage(`' + id + '`);" class="button btn bg-pink waves-effect"><i class="material-icons font-14">save</i> Done</div>'
					+ '<div onclick="closeModal()" id="close-crop-modal" class="button btn bg-brown waves-effect" aria-label="close"><i class="material-icons font-14">cancel</i> Cancel</div>'
					+ '</div>'
					+ '</div>'
					+ '<canvas style="position: absolute; top: -99999px; left: -99999px;" id="pollbanner-hidden-canvas"></canvas>'
		...
		$(this).click(function () {
			...
			$(this).one("change", function () {
				...
				if ($.inArray(ext, options.allowedInputs) == -1) {
					// $("#question_media_error").html('Invalid extension! Please check your input file and try again.');
					// $('#image').val('');
					// alert('invalid extension! Please check your input file and try again.');
					// resetFileInput(cropzeeInputId);
				} else {
					$("#question_video").html('');
					$("#question_media_error").html('');
