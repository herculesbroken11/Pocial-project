jQuery(document).ready(function ($) {
	jQuery("#sponsored_logo").sponsoredLogoCropzee();
});
...
jQuery.fn.extend({
	sponsoredLogoCropzee: function (options = {
	// croppr.js options
	// see https://jamesooi.design/Croppr.js/
		...
		window.cropzeeCreateSponsoredImage = function (id) {
			 
			$("#sponsored-cropzee-hidden-canvas").show();
			$(".sponsored_logo_imgae_hide").hide();
			...
			var canvas = document.getElementById('sponsored-cropzee-hidden-canvas');
		...
		// see https://jamesooi.design/Croppr.js/
		function cropzeeSponsoredTriggerCroppr() {
			window.cropzeeCroppr = new Croppr('#cropzee-modal-sponsored-image', {
		...
		function cropzeeSponsoredTriggerModal(id, src) {
			// $(".old_image_hide").hide();
			...
			var lightmodalHTML =
					...
					+ '<img id="cropzee-modal-sponsored-image" src="' + src + '" style="display:none;">'
					+ '</div>'
					+ '<!-- light modal footer -->'
					...
					+ '<div onclick="cropzeeCreateSponsoredImage(`' + id + '`);" class="button btn bg-brown waves-effect"><i class="material-icons font-14">save</i> Done</div>'
					+ '<div onclick="closeModal()" class="button btn bg-pink waves-effect" aria-label="close"><i class="material-icons font-14">cancel</i> Cancel</div>'
					+ '</div>'
					+ '</div>'
					+ '<canvas style="position: absolute; top: -99999px; left: -99999px;" id="sponsored-cropzee-hidden-canvas"></canvas>'
			...
			setTimeout(function () {
				...
				setTimeout(function () {
					...
					cropzeeSponsoredTriggerCroppr();
				}, 50);
			}, 50);
		}
		// function to capture input and insert it into various elements for previewing and display
		function cropzeeSponsoredReadURL(input, id) {
			$("#sponsored-cropzee-hidden-canvas").hide();
			if (input.files && input.files[0]) {
				...
				reader.onload = function (e) {
					...
					cropzeeSponsoredTriggerModal(id, e.target.result);
				};
		...
		$(this).click(function () {
			...
			$(this).one("change", function () {
				...
				if ($.inArray(ext, options.allowedInputs) == -1) {
					// $("#sponsored_error").html('Invalid extension! Please check your input file and try again.');
					// $('#sponsored_logo').val('');
					// alert('invalid extension! Please check your input file and try again.');
					// resetFileInput(cropzeeInputId);
				} else {
					$("#sponsored_temp_error").html('');
					...
					cropzeeSponsoredReadURL(this, cropzeeInputId);
				}
			});
		});
	}
