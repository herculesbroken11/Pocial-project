jQuery(document).ready(function ($) {
    // Event delegation for dynamically added elements starting with 'poll_options'
    $(document).on('change', '[id^="poll_options"]', function () {
        // Get the index from the ID of the changed element
        let indexAddMore = getIndexFromId($(this).attr('id'));

        // Dynamically select the element based on the index and call optionsPollsCropzee
        jQuery("#poll_options_" + indexAddMore).optionsPollsCropzee();
...
jQuery.fn.extend({
	optionsPollsCropzee: function (options = {
	// croppr.js options
	// see https://jamesooi.design/Croppr.js/
		...
		/** index get in image add more */
		let indexAddMore = getIndexFromId($(this).attr('id'));
		 console.log(indexAddMore)
		...
		window.cropzeeCreateOptionsImage = function (id) {
			 
			$("#option_src_image_thumbnail_"+indexAddMore).hide();
			...
			var canvas = document.getElementById('options-cropzee-hidden-canvas-'+indexAddMore);
			...
			if(dimensions.width < 200 && dimensions.height < 200){
				...
				imageLeftMargin = dimensions.width < 200 ? (200-dimensions.width)/2 : 0;
				imageTopMargin = dimensions.height < 200 ? (200-dimensions.height)/2: 0;
			}else{
			...
			ctx.translate(-dimensions.width / 2, -dimensions.height / 2);
			ctx.drawImage(img, dimensions.x, dimensions.y, dimensions.width, dimensions.height, imageLeftMargin, imageTopMargin, dimensions.width, dimensions.height);
			...
			if (options.returnImageMode == 'blob') {
				...
				$("#poll_options_binary_"+indexAddMore).val(dataURL);
				$("#thumbnail_option_video_image_"+indexAddMore).val('');
				document.getElementById("option_src_image_show_"+indexAddMore).src=dataURL;
			}
		...
		function cropzeeTriggerModal(id, src) {
			...
			var lightmodalHTML =
					...
					+ '<input id="upload-photo-image-rotate-value" type="text" size="1" name="rangeInputValue" value="0">'
					...
					+ '<div onclick="cropzeeCreateOptionsImage(`' + id + '`);" class="button btn bg-brown waves-effect"><i class="material-icons font-14">save</i> Done</div>'
					+ '<div onclick="closeModal()" id="close-crop-modal" class="button btn bg-pink waves-effect" aria-label="close"><i class="material-icons font-14">cancel</i> Cancel</div>'
					+ '</div>'
					+ '</div>'
					+ '<canvas style="position: absolute; top: -99999px; left: -99999px;" id="options-cropzee-hidden-canvas-'+indexAddMore+'"></canvas>'
		...
		function cropzeeReadURL(input, id) {
			$("#options-cropzee-hidden-canvas-"+indexAddMore).hide();
		...
		$(document).click(function (e) {
			...
			if ($(e.target).is('#close-crop-modal')) {
				location.reload()
			}
		});
		...
		$(this).click(function () {
			...
			$(this).one("change", function () {
				...
				if ($.inArray(ext, options.allowedInputs) == -1) {
					//document.getElementById("option_src_image_show").src= $('#video_logo_image').val();
					// $("#options_image_error").html('Invalid extension! Please check your input file and try again.');
					// $('#poll_options').val('');
					// alert('invalid extension! Please check your input file and try again.');
					// resetFileInput(cropzeeInputId);
				} else {
					$("#business_banner_temp_error").html('');
					$("#options_image_error_"+indexAddMore).html('');
...
// Function to extract index from ID
function getIndexFromId(id) {
	// Extract numerical part from the ID using regex
	var match = id.match(/\d+/);
	// Convert the matched part to a number
	if (match) {
