jQuery(document).ready(function ($) {
		jQuery("#business_logo").businessLogoCropzee();
	});
	...
	jQuery.fn.extend({
		businessLogoCropzee: function (options = {
		// croppr.js options
		// see https://jamesooi.design/Croppr.js/
			...
			window.cropzeeCreateLogoImage = function (id) {
				 
				$("#business-logo-cropzee-hidden-canvas").show();
				$(".businessLogo").hide();
				...
				var canvas = document.getElementById('business-logo-cropzee-hidden-canvas');
				...
				if (options.returnImageMode == 'blob') {
					...
					$("#business_logo_binary").val(dataURL)
				}
			...
			function cropzeeTriggerModal(id, src) {
				...
				var lightmodalHTML =
						...
						+ '<div onclick="cropzeeCreateLogoImage(`' + id + '`);" class="button btn bg-brown waves-effect"><i class="material-icons font-14">save</i> Done</div>'
						...
						+ '</div>'
						+ '</div>'
						+ '<canvas style="position: absolute; top: -99999px; left: -99999px;" id="business-logo-cropzee-hidden-canvas"></canvas>'
			...
			function cropzeeReadURL(input, id) {
				$("#business-logo-cropzee-hidden-canvas").hide();
			...
			$(this).click(function () {
				...
				$(this).one("change", function () {
					...
					if ($.inArray(ext, options.allowedInputs) == -1) {
						// $("#business_logo_error").html('Invalid extension! Please check your input file and try again.');
						// $('#business_logo').val('');
						// alert('invalid extension! Please check your input file and try again.');
						// resetFileInput(cropzeeInputId);
					} else {
						$("#business_logo_temp_error").html('');
