		...
		cropzee: function (options = {
			...
			window.cropzeeCreateImage = function (id) {
				...
				if (options.returnImageMode == 'blob') {
					...
					$("#background_image_binary").val(dataURL)
				}
			...
			function cropzeeTriggerCroppr() {
				...
				/** Start dynamic height width set image cropper*/
				var e = document.getElementsByName("embeded_size")[0];
				var selectedValue = e.options[e.selectedIndex].value;
				
				let startSizeDynamic = options.startSize;
				if(selectedValue=='custom_size'){
					let width	= document.getElementById("width").value;
					let height	= document.getElementById("height").value;
					
					let integerIframeWidth	= parseFloat(width, 10);
					let integerIframeHeight	= parseFloat(height, 10);
					
					let imagePixels  = width.split(integerIframeWidth);
					imagePixels = (imagePixels && imagePixels[1]) ? (imagePixels[1]).toLowerCase() : "px";
					
					startSizeDynamic = (integerIframeWidth && integerIframeHeight) ? [integerIframeWidth, integerIframeHeight, imagePixels] : startSizeDynamic;
					
				}
				if(selectedValue && selectedValue!='custom_size'){
					let splitValue	= selectedValue.split('_');
					startSizeDynamic	= [Number(splitValue[0]), Number(splitValue[1]), 'px'];
				}
				/** End dynamic height width set image cropper*/
				
				window.cropzeeCroppr = new Croppr('#cropzee-modal-image', {
					aspectRatio: options.aspectRatio,
					maxSize: options.maxSize,
					minSize: options.minSize,
					startSize: startSizeDynamic,
			...
			function cropzeeTriggerModal(id, src) {
				...
				var lightmodalHTML =
						...
						+ '<input id="upload-photo-image-rotate-value" size="1" type="text" name="rangeInputValue" value="0">'
			...
			$(this).click(function () {
				...
				$(this).one("change", function () {
					...
					if ($.inArray(ext, options.allowedInputs) == -1) {
						// $("#background_image_error").html('Invalid extension! Please check your input file and try again.');
						// $('#background_image').val('');
						// alert('invalid extension! Please check your input file and try again.');
						// resetFileInput(cropzeeInputId);
					} else {
						$("#background_image_temp_error").html('');
