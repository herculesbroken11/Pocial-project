jQuery(document).ready(function ($) {
	jQuery("#poll_options").optionsPollsCropzee();
});
...
jQuery.fn.extend({
	...
	// see https://jamesooi.design/Croppr.js/
		...
		window.cropzeeCreateOptionsImage = function (id) {
			...
			$("#option_src_image_thumbnail").hide();
			...
			var canvas = document.getElementById('options-cropzee-hidden-canvas');
			...
			if (options.returnImageMode == 'blob') {
				...
				$("#poll_options_binary").val(dataURL);
				$("#thumbnail_option_video_image").val('');
				document.getElementById("option_src_image_show").src=dataURL;
			}
			closeModal();
		};
		...
		function cropzeeTriggerModal(id, src) {
			...
			var lightmodalHTML =
					...
					+ '</div>'
					+ '</div>'
					+ '<canvas style="position: absolute; top: -99999px; left: -99999px;" id="options-cropzee-hidden-canvas"></canvas>'
		...
		function cropzeeReadURL(input, id) {
			$("#options-cropzee-hidden-canvas").hide();
