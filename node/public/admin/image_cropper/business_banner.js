jQuery(document).ready(function ($) {
		jQuery("#business_banner").businessBannerCropzee();
	});
	...
	jQuery.fn.extend({
		businessBannerCropzee: function (options = {
		// croppr.js options
		// see https://jamesooi.design/Croppr.js/
			...
			window.cropzeeCreateBannerImage = function (id) {
				 
				$("#business-banner-cropzee-hidden-canvas").show();
				$(".businessBanner").hide();
				...
				var canvas = document.getElementById('business-banner-cropzee-hidden-canvas');
				...
				if (options.returnImageMode == 'blob') {
					...
					$("#business_banner_binary").val(dataURL)
				}
			...
			function cropzeeTriggerModal(id, src) {
				...
				var lightmodalHTML =
						...
						+ '<div onclick="cropzeeCreateBannerImage(`' + id + '`);" class="button btn bg-brown waves-effect"><i class="material-icons font-14">save</i> Done</div>'
						...
						+ '</div>'
						+ '</div>'
						+ '<canvas style="position: absolute; top: -99999px; left: -99999px;" id="business-banner-cropzee-hidden-canvas"></canvas>'
			...
			function cropzeeReadURL(input, id) {
				$("#business-banner-cropzee-hidden-canvas").hide();
			...
			$(this).click(function () {
				...
				$(this).one("change", function () {
					...
					if ($.inArray(ext, options.allowedInputs) == -1) {
						// $("#business_banner_error").html('Invalid extension! Please check your input file and try again.');
						// $('#business_banner').val('');
						// alert('invalid extension! Please check your input file and try again.');
						// resetFileInput(cropzeeInputId);
					} else {
						$("#business_banner_temp_error").html('');
						$("#business_banner_error").html('');
