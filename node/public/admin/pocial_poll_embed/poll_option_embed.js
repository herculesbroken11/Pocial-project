...
// let PocialEmbedFrontUrl_{{CURRENT_SCRIPT_ID}} = 'https://www.pocial.com/';
...
let ownerPollUserId_{{CURRENT_SCRIPT_ID}} = "";
let registrationLeadFormsId_{{CURRENT_SCRIPT_ID}} = "";
...
function ViewEmbedGenerateDetails_{{CURRENT_SCRIPT_ID}}() {
	return new Promise(resolve => {
		...
		let articleEmbedPollId = elem.getAttribute('data-article_embed_poll_id');

		const makeJsonGenerateEmbedString = sendObjectString_{{CURRENT_SCRIPT_ID}};
		Object.assign(makeJsonGenerateEmbedString.data, { "embed_generate_slug": embedGenerateSlug, "article_embed_poll_id": articleEmbedPollId});
...
function viewPublicPagePollDetails_{{CURRENT_SCRIPT_ID}}() {
	return new Promise(resolve => {
		...
		let elem = document.getElementById("pocialEmbedPollContainer_{{CURRENT_SCRIPT_ID}}");
		let articleEmbedPollId = elem.getAttribute('data-article_embed_poll_id');
		let articleEmbedPollOptionId = elem.getAttribute('data-article_embed_poll_option_id');

		let pollDetailsString = sendObjectString_{{CURRENT_SCRIPT_ID}};
		Object.assign(pollDetailsString.data, { "custom_url": customUrl_{{CURRENT_SCRIPT_ID}}, "random_views_string": uniqueViewRandomString_{{CURRENT_SCRIPT_ID}}, "article_embed_poll_id": articleEmbedPollId, "article_embed_poll_option_id": articleEmbedPollOptionId});
...
async function pocialPollMainEmbedCodeGenerate_{{CURRENT_SCRIPT_ID}} () {
	...
	ownerPollUserId_{{CURRENT_SCRIPT_ID}} = (pollResult && pollResult.owner_poll_user_id) ? pollResult.owner_poll_user_id : "";
	registrationLeadFormsId_{{CURRENT_SCRIPT_ID}} = (pollResult && pollResult.registration_lead_forms_id) ? pollResult.registration_lead_forms_id : "";
	...
	if (pollDetails && pollDetails.result && (Object.keys(pollDetails.result).length == 0 || Object.keys(embedGenerateResult).length == 0)) {
		...
		pollOptionHtml += '<div class="pocial_no_record">No Embed Found'+
				...
				// } else {
					pollOptionHtml += '<li class="pocial_text_option_li '+ctaLinkShowClass+'">' +
							// '<div class="formcheck text-option '+fullProgressBg+'">' +
								// ((!voteCheck) ? '<input class="pocial-formcheck-input pointer" type="radio" onclick=pocialPollVoting_{{CURRENT_SCRIPT_ID}}('+pocialObjectVotingData+') name="flexRadioDefault" id="' + optinId + '">':"") +
								((!voteCheck) ? '<label onclick=pocialPollVoting_{{CURRENT_SCRIPT_ID}}('+pocialObjectVotingData+')  class="pocial-formcheck-label hyperlink_text_option" for="' + optinId + '">' + records.title + '</label>':"") +
								((voteCheck) ? '<div class="progress_embed">'+
													'<label onclick=pocialAlreadyPollVoting_{{CURRENT_SCRIPT_ID}}() class="pocial-formcheck-label" for="' + optinId + '">' + records.title + '</label>' +
	...
	pocialGenerateCode_{{CURRENT_SCRIPT_ID}} = function () {
		...
		jQuery("<div>").attr({ class: "article_embed_option_hyperlink_text embed_parent_1669032135160" }).html('<div class="page_custom_heightt ' + singleOptionSubmittedType + '">' +
				'<div class="free_lunchh '+singleMediaPollsClass+'">' +
				...
				'<div class="pocial-col-md-6 reward_right_text">'+
					'<div class="right-box">'+
						...
						'<span id="pocial_reward_enticement_replace_html">'+
							...
							'</a>'+
							'<button class="sign-in pointer" onclick="openRegistrationPopup_{{CURRENT_SCRIPT_ID}}()" >'+
		...
		}).html('<div class="article_embed_forget"><img src="'+frontAssetsImageUrl_{{CURRENT_SCRIPT_ID}}+'logo.svg" class="pocial_logo_forget_popup" alt="img">'+
		'<div class="pocial_cross_enticement"><a href="javascript:void(0)" onclick=pocialCrossEnticementHtml_{{CURRENT_SCRIPT_ID}}()><img src="'+pocialCrossIconImage_{{CURRENT_SCRIPT_ID}}+'" alt="img"></a></div>'+
		...
		}).html('<div class="article_embed_reset_password_forget"><img src="'+frontAssetsImageUrl_{{CURRENT_SCRIPT_ID}}+'logo.svg" class="pocial_logo_reset_popup" alt="img">'+
				'<div class="pocial_cross_enticement"><a href="javascript:void(0)" onclick=pocialCrossEnticementHtml_{{CURRENT_SCRIPT_ID}}()><img src="'+pocialCrossIconImage_{{CURRENT_SCRIPT_ID}}+'" alt="img"></a></div>'+
...
/** Function for used already poll voting */
pocialAlreadyPollVoting_{{CURRENT_SCRIPT_ID}} = function () {
	showErrorMessage_{{CURRENT_SCRIPT_ID}}(pocialTextSettings_{{CURRENT_SCRIPT_ID}}['front.polls.you_have_already_submitted_your_vote']);
}
	...
	xhttp.onload = function () {
		...
		if(pocialVotingPollData && loginCrediantialsEmbed && !assignReward_{{CURRENT_SCRIPT_ID}}){
			...
			if (pocialVotingPollData && assignReward_{{CURRENT_SCRIPT_ID}}) {
				jQuery("body").addClass("body_ai_article_css");
				jQuery("#pocialEmbedPollContainer_{{CURRENT_SCRIPT_ID}} .embed_parent_1669032135160").addClass("pocial_after_vote");
				jQuery("#pocialEmbedPollContainer_{{CURRENT_SCRIPT_ID}}").removeClass("articleEmbedPollOption");
...
pocialCrossEnticementHtml_{{CURRENT_SCRIPT_ID}} = function () {
	/** Main function call generate */
	jQuery("#pocialEmbedPollContainer_{{CURRENT_SCRIPT_ID}}").addClass("articleEmbedPollOption");
	...
	jQuery("body").removeClass("body_ai_article_css");
...
pocialEmbedRegistration_{{CURRENT_SCRIPT_ID}} = function () {
	...
	let pocialGender = jQuery('#pocialEmbedPollContainer_{{CURRENT_SCRIPT_ID}} input[name="pocial_gender"]:checked').val();
	let PocialMonth = jQuery("#pocialEmbedPollContainer_{{CURRENT_SCRIPT_ID}} #mm_{{CURRENT_SCRIPT_ID}}").val();
	let pocialDate = jQuery("#pocialEmbedPollContainer_{{CURRENT_SCRIPT_ID}} #dd_{{CURRENT_SCRIPT_ID}}").val();
	let pocialYear = jQuery("#pocialEmbedPollContainer_{{CURRENT_SCRIPT_ID}} #yy_{{CURRENT_SCRIPT_ID}}").val();
	...
	let pocialMobile = jQuery("#pocialEmbedPollContainer_{{CURRENT_SCRIPT_ID}} #pocial_mobile_{{CURRENT_SCRIPT_ID}}").val();
	let pocialZipCode = jQuery("#pocialEmbedPollContainer_{{CURRENT_SCRIPT_ID}} #pocial_zip_code_{{CURRENT_SCRIPT_ID}}").val();
	...
	if (errorRegistrationValidation == true) {
		...
		Object.assign(makeJsonRegistrationString.data, {"first_name":pocialFirstName,"last_name":pocialLastName,"email":pocialEmail,"password":pocialPassword,"dob":pocialDob,"user_name":pocialUsername,"zip":pocialZipCode,"mobile":pocialMobile,"gender":pocialGender,"social_id":"","provider":"","registered_from":1, "user_source_type":"poll", "owner_poll_user_id":ownerPollUserId_{{CURRENT_SCRIPT_ID}}, "registration_lead_forms_id":registrationLeadFormsId_{{CURRENT_SCRIPT_ID}} });
...
function changeSizeContainer_{{CURRENT_SCRIPT_ID}}() {
	...
	var elemWidth_{{CURRENT_SCRIPT_ID}} = document.body.offsetWidth;
...
var randomAlphaNum = Math.floor(Math.random() * 10000) + 1;
