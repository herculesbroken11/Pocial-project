...
let pollDeleted_{{CURRENT_SCRIPT_ID}} = "";
let pollDeactive_{{CURRENT_SCRIPT_ID}} = "";
...
async function pocialPollMainEmbedCodeGenerate_{{CURRENT_SCRIPT_ID}} () {
	...
	pollDeleted_{{CURRENT_SCRIPT_ID}} = (embedGenerateResult.embed_delete) ? embedGenerateResult.embed_delete : 0;	
	pollDeactive_{{CURRENT_SCRIPT_ID}} = (embedGenerateResult.embed_active) ? embedGenerateResult.embed_active : 0;	
	...
	} else if (pollDeleted_{{CURRENT_SCRIPT_ID}}==1) {
	 
	 	/** Delete poll emebd */
		pollOptionHtml += '<div class="pocial_no_record">'+
								// '<figure ><img alt="image" class="righticon" src="'+frontAssetsImageUrl_{{CURRENT_SCRIPT_ID}}+'no-record-found.svg"></figure>'+
								'<h1>'+pocialTextSettings_{{CURRENT_SCRIPT_ID}}['poll_embed.this_poll_embed_is_deleted']+'</h1>'+
							'</div>';
						
	} else if (pollDeactive_{{CURRENT_SCRIPT_ID}}==0) {
	 
	 	/** active/deactve poll emebd */
		pollOptionHtml += '<div class="pocial_no_record">'+
								// '<figure ><img alt="image" class="righticon" src="'+frontAssetsImageUrl_{{CURRENT_SCRIPT_ID}}+'no-record-found.svg"></figure>'+
								'<h1>'+pocialTextSettings_{{CURRENT_SCRIPT_ID}}['poll_embed.this_poll_embed_is_deactivated']+'</h1>'+
							'</div>';	
	...
	pocialGenerateCode_{{CURRENT_SCRIPT_ID}} = function () {
		...
		jQuery("<div>").attr({ class: "embed_parent_1669032135160" }).html('<div class="page_custom_height ' + singleOptionSubmittedType + '">' +
				'<div class="free_lunch '+singleMediaPollsClass+'">' +
					...
					'<div class="free_lunch_detail pocial_poll_question_and_options">' +
						((pollDeleted_{{CURRENT_SCRIPT_ID}}==0 && pollDeactive_{{CURRENT_SCRIPT_ID}}==1) ?  '<h2>' + questionQuestion + '</h2>'  : "") +
